...
class JDService {
  modifyUserObjectForJD(user, loggedInUserId) {
    ...
    new_user[values.JD.IMAGE_URL] = `/user_images/${new_user["cn"]}.${jdConfig.DEFAULT_USER_IMAGE_FILE_EXTENSION}`;
  ...
  async updateUserData(userData, loggedInUserId) {
    ...
    if (!userData[values.JD.EDIT_PROFILE_ACCESS])
      throw new Error(values.ERROR.EDIT_PROFILE_ACCESS_MISSING);
  ...
  async saveUserImage(userData, imageFile, loggedInUserId) {
